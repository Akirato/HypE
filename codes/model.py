...
import pickle
import math
from manifolds.base import ManifoldParameter, ManifoldTensor
...
class Query2Manifold(nn.Module):
    ...
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma, 
        ...
        if trainable_curvature:
            self.c = nn.Parameter(torch.FloatTensor([self.float_c]),requires_grad=True)
        else:
            self.c = nn.Parameter(torch.FloatTensor([self.float_c]),requires_grad=False)
    ...
    def BoxTransE(self, head, relation, tail, mode, offset, head_offset, rel_len, qtype):
        ...
        return score, ManifoldTensor(score_center,self.float_c), torch.mean(torch.norm(offset, p=2, dim=2).squeeze(1)), ManifoldTensor(score_center_plus,self.float_c), None
